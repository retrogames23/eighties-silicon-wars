        ...
        <CardContent className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              ...
              {(['step3', 'step4', 'step5'] as const).map((key, idx) => (
                <div key={key} className="flex items-start gap-3 p-3 bg-accent/10 rounded">
                  <div className="bg-neon-green text-black rounded-full w-6 h-6 flex items-center justify-center text-sm font-bold">{idx + 3}</div>
                  <div>
                    <h4 className="font-bold">{t(`tutorial:gameplay.${key}.title`)}</h4>
                    <p className="text-sm text-muted-foreground">
                      {t(`tutorial:gameplay.${key}.description`)}
                    </p>
                  </div>
                </div>
              ))}
          ...
          <div className="space-y-3">
            <h3 className="text-xl font-bold text-neon-cyan flex items-center gap-2">
              ...
              {t('tutorial:hardware.title')}
            </h3>
            <p className="text-sm text-muted-foreground">{t('tutorial:hardware.intro')}</p>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="bg-accent/10 p-3 rounded">
                <h4 className="font-bold text-neon-green mb-2">{t('tutorial:hardware.components.title')}</h4>
                <ul className="text-sm space-y-1 text-muted-foreground">
                  {(t('tutorial:hardware.components.points', { returnObjects: true }) as string[]).map((p, i) => (
                    <li key={i}>• {p}</li>
                  ))}
                </ul>
              </div>
              <div className="bg-accent/10 p-3 rounded">
                <h4 className="font-bold text-yellow-400 mb-2">{t('tutorial:hardware.tip.title')}</h4>
                <p className="text-sm text-muted-foreground">{t('tutorial:hardware.tip.description')}</p>
              </div>
            </div>
          </div>
            ...
            <div className="bg-accent/10 p-4 rounded space-y-2">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 text-sm">
                <div>
                  ...
                  <p className="text-muted-foreground">{t('tutorial:budget.marketing.description')}</p>
                </div>
                <div>
                  <h4 className="font-bold text-blue-400">{t('tutorial:budget.development.title')}</h4>
                  <p className="text-muted-foreground">{t('tutorial:budget.development.description')}</p>
                </div>
                <div>
                  <h4 className="font-bold text-purple-400">{t('tutorial:budget.research.title')}</h4>
                  <p className="text-muted-foreground">{t('tutorial:budget.research.description')}</p>
                </div>
                <div>
                  <h4 className="font-bold text-pink-400">{t('tutorial:budget.support.title')}</h4>
                  <p className="text-muted-foreground">{t('tutorial:budget.support.description')}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-3">
            <h3 className="text-xl font-bold text-neon-cyan flex items-center gap-2">
              <Users className="w-5 h-5" />
              {t('tutorial:staff.title')}
            </h3>
            <p className="text-sm text-muted-foreground">{t('tutorial:staff.intro')}</p>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
              {(['engineer', 'marketer', 'researcher', 'support'] as const).map((r) => (
                <div key={r} className="p-2 bg-accent/10 rounded text-muted-foreground">
                  • {t(`tutorial:staff.roles.${r}`)}
                </div>
              ))}
            </div>
            <p className="text-xs text-yellow-400/80">{t('tutorial:staff.tip')}</p>
          </div>
          ...
          <div className="space-y-3">
            <h3 className="text-xl font-bold text-neon-cyan flex items-center gap-2">
              ...
              {t('tutorial:financing.title')}
            </h3>
            <p className="text-sm text-muted-foreground">{t('tutorial:financing.intro')}</p>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-sm">
              {(['bootstrap', 'loan', 'vc'] as const).map((o) => (
                <div key={o} className="p-3 bg-accent/10 rounded text-muted-foreground">
                  {t(`tutorial:financing.options.${o}`)}
                </div>
              ))}
            </div>
          </div>
